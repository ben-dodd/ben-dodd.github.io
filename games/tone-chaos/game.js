...
function createRandomVoice(synth) {
  ...
  synth.envelope.attack = random(0, 200) / 100
  // Change envelope.decay
  synth.envelope.decay = random(0, 300) / 100
  // Change envelope.sustain
  synth.envelope.sustain = random(0, 100) / 100
  // Change envelope.release
  synth.envelope.release = random(0, 1000) / 100
  // Change modulation.type
  synth.modulation.type = `${
    ['sine', 'square', 'sawtooth', 'triangle'][random(0, 4)]
  }`
  // Change modulation.attack
  synth.modulation.attack = random(0, 200) / 100
  // Change modulation.decay
  synth.modulation.decay = random(0, 300) / 100
